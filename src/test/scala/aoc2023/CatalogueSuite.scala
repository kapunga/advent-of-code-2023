...
class CatalogueSuite extends CatsEffectSuite {
  ...
  val partOneSolution: String = "35"

  val partTwoSolution: String = "46"

  test("Part one solution works against example.") {
    Catalogue.lookup(fs2.Stream.emits(partOneExample))
      .flatMap(result => IO(assertEquals(result, partOneSolution)))
  }

  test("Part two solution works against example.") {
    Catalogue.lookupRange(fs2.Stream.emits(partOneExample))
