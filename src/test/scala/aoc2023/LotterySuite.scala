...
class LotterySuite extends CatsEffectSuite {
  ...
  val partOneSolution: String = "13"
  
  val partTwoSolution: String = "30"

  test("Part one solution works against example.") {
    Lottery.points(fs2.Stream.emits(partOneExample))
      .flatMap(result => IO(assertEquals(result, partOneSolution)))
  }

  test("Part two solution works against example.") {
    Lottery.totalTickets(fs2.Stream.emits(partOneExample))
